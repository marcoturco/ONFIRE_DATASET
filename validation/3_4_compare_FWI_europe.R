...
for (i in 1:length(lon_com)) {
  for (j in 1:length(lat_com)) {
    ...
    FWI_clean <- FWI[i, j, ][complete.cases(FWI[i, j, ])]
    n_fwi <- length(FWI_clean)
    if (n_obs > 1 & n_fwi > 1) {
...
# setEPS()
# postscript(paste0(dir_out,"figures/corr_fwi_europe.eps"), width=11,height=8.5)
...
save(list=c("lon", "lat", "corre","pvalue"), file=paste0(dir_out,"ForFigures/ForFig7c.RData"))
...
# iok=which(pvalue<=0.05)
# setEPS()
# postscript(paste0(dir_out,"figures/corr_fwi_europe_f51.eps"), width=11,height=8.5)
...
lon=lon_com
lat=lat_com
save(list=c("lon", "lat", "corre","pvalue"), file=paste0(dir_out,"ForFigures/ForFig7f.RData"))
