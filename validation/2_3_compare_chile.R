...
setEPS()
postscript(paste0(dir_out,"figures/mean_BA_chile_ONFIRE.eps"), width=4,height=12)
image(lon,lat,(toplot), breaks = my_breaks, col = my_col, legend = FALSE, zlim=zlim)
plot(wrld_simpl, add = TRUE)
dev.off()

toplot = apply(BA_y,c(1,2),mean,na.rm=T)
save(list=c("lon", "lat", "toplot"), file=paste0(dir_out,"ForFigures/ForFig3c.RData"))
...
toplot[toplot>zlim[2]]=zlim[2]
setEPS()
postscript(paste0(dir_out,"figures/mean_BA_chile_F51.eps"), width=4,height=12)
image(lon,lat,(toplot), breaks = my_breaks, col = my_col, legend = FALSE, zlim=zlim)
plot(wrld_simpl, add = TRUE)
dev.off()
...
toplot = apply(F51_y/1000000,c(1,2),mean,na.rm=T)
save(list=c("lon", "lat", "toplot"), file=paste0(dir_out,"ForFigures/ForFig3d.RData"))
...
# setEPS()
# postscript(paste0(dir_out,"figures/corr_chile_sort_period.eps"), width=4,height=12)
# image(lon,lat,(toplot), breaks = my_breaks, col = my_col, legend = FALSE, zlim=zlim)
...
save(list=c("lon", "lat", "corre","pvalue"), file=paste0(dir_out,"ForFigures/ForFig5b.RData"))
