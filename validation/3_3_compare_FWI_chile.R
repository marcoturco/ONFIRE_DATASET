...
## chile
# load grid 
load(paste0(dir_out, "CHILE/BA_CHILE_v1.RData"))
load(paste0(dir_out, "CHILE/lon_BA_CHILE_v1.RData"))
load(paste0(dir_out, "CHILE/lat_BA_CHILE_v1.RData"))
ilon=match(lon,lon_com)
ilat=match(lat,lat_com)
iok <- seq(as.Date("1985-01-01"), as.Date("2021-12-01"), by = "month")
...
# setEPS()
# postscript(paste0(dir_out,"figures/corr_fwi_chile.eps"), width=4,height=12)
...
save(list=c("lon", "lat", "corre","pvalue"), file=paste0(dir_out,"ForFigures/ForFig7b.RData"))
...
for (i in 1:length(lon_com)) {
  for (j in 1:length(lat_com)) {
    if (sum(BA_onfire[i, j,],na.rm=T) >0 & sum(F51[i, j,],na.rm=T) >0 & sum(FWI[i, j,],na.rm=T) >0) {
      dum = cor.test(
        F51[i, j,],
        FWI[i, j,],
        use = "pairwise.complete.obs",
...
# iok=which(pvalue<=0.05)
# setEPS()
# postscript(paste0(dir_out,"figures/corr_fwi_chile_f51.eps"), width=11,height=8.5)
...
aux[pvalue>0.05] = -999
aux[!is.na(corre2)] = corre2[!is.na(corre2)]
postscript(paste0(dir_out,"figures/corr_fwi_chile_f51.eps"), width=4,height=12)
image_mask(
  lon_com,
  lat_com,
  aux,
...
lon=lon_com
lat=lat_com
save(list=c("lon", "lat", "corre","pvalue"), file=paste0(dir_out,"ForFigures/ForFig7e.RData"))
