...
dir_fire = '/Users/marco/Documents/dati/fire_climate_data/fire/australia/tasmania/LIST_FIRE_HISTORY_STATEWIDE/'

## load data
mydata<- readOGR(dir_fire, "list_fire_history_statewide", verbose = FALSE)
mydata <-
  spTransform(mydata,
              CRS(
...
ba = mydata$SHAPE_AREA/10000 # in hectar
centroids <- getSpPPolygonsLabptSlots(mydata)
lon_fire=centroids[,1]
lat_fire=centroids[,2]
anni = format(as.Date(mydata$IGN_DATE, format="%Y/%m/%d"),"%Y")
anni=as.numeric(as.character(anni))
mesi = format(as.Date(mydata$IGN_DATE, format="%Y/%m/%d"),"%m")
