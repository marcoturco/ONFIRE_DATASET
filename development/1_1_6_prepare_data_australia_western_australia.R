...
dir_fire = '/Users/marco/Documents/dati/fire_climate_data/fire/australia/Western_Australia/DBCA_Fire_History_DBCA_060_WA_GDA2020_Public_Shapefile/'


## load data
mydata<- readOGR(dir_fire, "DBCA_Fire_History_DBCA_060")
...
ba = mydata$fih_hectar
# centroids <- st_centroid(mydata)
centroids <- getSpPPolygonsLabptSlots(mydata)


lon_fire=centroids[,1]
lat_fire=centroids[,2]
anni = format(as.Date(mydata$fih_date1, format="%Y%m%d"),"%Y")
anni=as.numeric(as.character(anni))
mesi = format(as.Date(mydata$fih_date1, format="%Y%m%d"),"%m")
