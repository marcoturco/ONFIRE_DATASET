...
dir_out = '/Users/marco/Dropbox/model/fire_database/out_def/CANADA/'
dir_obs <- "/Users/marco/Documents/dati/fire_climate_data/fire/"

# Load fire data from NFDB
mydata<- readOGR(paste0(dir_obs, "fire_canada/NFDB_point_large_fires/NFDB_point_20220901_large_fires.shp"), verbose = FALSE)
mydata@coords <- as.matrix(mydata@data[c("LONGITUDE","LATITUDE")])
mydata@bbox   <- rbind(range(mydata@coords[,1]),range(mydata@coords[,2]))
proj4string(mydata) <- CRS.new
# Get fire data attributes
ba = mydata$SIZE_HA
lon_fire=mydata$LONGITUDE
lat_fire=mydata$LATITUDE
anni = mydata$YEAR
...
ncatt_put(
  ...
  0,
  "Title",
  'An integrated dataset of observed national inventories of fires - CANADA NFDB'
)
