...
dir_fire = '/Users/marco/Documents/dati/fire_climate_data/fire/australia/South_Australia/FIREMGT_FireHistory_shp/'

## load data
mydata<- readOGR(dir_fire, "FIREMGT_FireHistory_GDA2020", verbose = FALSE)
mydata <-
  spTransform(mydata,
              CRS(
...
anni = format(as.Date(mydata$FIREDATE, format="%Y/%m/%d"),"%Y")
anni=as.numeric(as.character(anni))
mesi = format(as.Date(mydata$FIREDATE, format="%Y/%m/%d"),"%m")
