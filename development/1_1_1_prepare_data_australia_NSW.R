...
dir_fire = '/Users/marco/Documents/dati/fire_climate_data/fire/australia/New_South_Wales/fire_npwsfirehistory/'

## load data
mydata<- readOGR(dir_fire, "NPWS_FireHistory", verbose = FALSE)
mydata <-
  spTransform(mydata,
              CRS(
...
anni = format(as.Date(mydata$StartDate, format="%Y/%m/%d"),"%Y")
anni=as.numeric(as.character(anni))
mesi = format(as.Date(mydata$StartDate, format="%Y/%m/%d"),"%m")
