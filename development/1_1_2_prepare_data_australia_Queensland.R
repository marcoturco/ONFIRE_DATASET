...
dir_fire = '~/Documents/dati/fire_climate_data/fire/australia/Queensland/'


years=1950:2021
## load data
mydata<- readOGR(dir_fire, "Fire_history___QPWS", verbose = FALSE)
mydata <-
  spTransform(mydata,
              CRS(
...
# https://qldspatial.information.qld.gov.au/catalogue/custom/search.page?q=%22Fire%20history%20-%20Queensland%20Parks%20and%20Wildlife%20Service%22
ba = mydata$AREA_HA 
centroids <- getSpPPolygonsLabptSlots(mydata)
lon_fire=centroids[,1]
lat_fire=centroids[,2]
anni = format(as.Date(mydata$IGNITIONDA, format="%Y/%m/%d"),"%Y")
anni=as.numeric(as.character(anni))
mesi = format(as.Date(mydata$IGNITIONDA, format="%Y/%m/%d"),"%m")
